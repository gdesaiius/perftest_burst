...
bool RTIDDSImpl<T>::ParseConfig(int argc, char *argv[])
{
    ...
    for (i = 0; i < argc; ++i) {
        ...
        else if (IS_OPTION(argv[i], "-burstDelay"))
        {
            if ((i == (argc-1)) || *argv[++i] == '-')
            {
                fprintf(stderr, "Missing <delay> after -burstDelay\n");
                return false;
            }
        }
        else if (IS_OPTION(argv[i], "-burstSamples"))
        {
            if ((i == (argc-1)) || *argv[++i] == '-')
            {
                fprintf(stderr, "Missing <num of samples> after -burstSamples\n");
                return false;
            }
        }
...
bool RTIDDSImpl<T>::Initialize(int argc, char *argv[])
{
    ...
    if (_factory->get_participant_qos_from_profile(
        ...
        fprintf(
                stderr,
                "Problem getting QoS Library \"%s::BaseProfileQos\" for participant_qos.\n",
                _ProfileLibraryName);
    }
   ...
   /* Enable Monitor.*/
   /*
    if (DDSPropertyQosPolicyHelper::add_property(
                qos.property,"rti.monitor.library", "rtimonitoring", DDS_BOOLEAN_FALSE) == DDS_RETCODE_OK) {
        if (DDSPropertyQosPolicyHelper::add_pointer_property(qos.property,
                        "rti.monitor.create_function_ptr",
                        (void *)RTIDefaultMonitor_create) == DDS_RETCODE_OK) {
            printf("Monitor enabled\n");
        }
        else {
            printf ("Error setting monitor create_function\n");
        }
    }
    else {
